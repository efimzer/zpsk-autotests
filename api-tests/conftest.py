...
from config.settings import BASE_URL, CODE, GROUP_CHAT_TITLE, GROUP_INITIAL_PARTICIPANTS_PHONES, PHONE, PHONE_PARTICIPANT

# API-клиент - это созданный объект класса ApiClient со своими base_url, timeout и session.
# Через этот объект можно вызывать методы login, get_auth_me, get_chats и другие.
@pytest.fixture
def api_client():
...
@pytest.fixture
def sent_message(authorized_api_client, attach_api_info, created_chat):
...
@pytest.fixture
def chats(authorized_api_client, attach_api_info):
